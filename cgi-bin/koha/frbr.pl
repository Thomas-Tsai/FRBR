...
my ( $tmpl, $loggedinuser, $cookie ) = get_template_and_user(
    {
	template_name   => "cataloguing/frbr.tmpl",
	query           => $input,
	type            => "intranet",
	authnotrequired => 0,
...
sub get_current_relation{
	...
	if ($found == 1){
	    ...
	    #data{'type'} = $rt->{'type'};
	    if ($link eq 'right'){
		$data{'desc'} = get_left_relation_type_desc($rt->{'type'});
	    } else {
...
sub main {
    ...
    output_html_with_http_headers $input, $cookie, $tmpl->output;
